...
import com.example.refugerestrooms.firebase.data.UserData
import com.example.refugerestrooms.firebase.data.createUserData
...
class MainViewModel: ViewModel() {
    ...
    fun writeToDatabase(latitude: Double, longitude: Double) {
        val user = _signedInUser.value
        user?.run {
            val userIdReference = Firebase.database.reference
                .child("users").child(uid)
            val userData = createUserData(displayName!!, latitude, longitude)
